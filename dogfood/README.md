...
- [`devswitchboard-v0.1-release-exit-016.json`](devswitchboard-v0.1-release-exit-016.json) records the frozen v0.1 release-exit audit, the developer-classified Approved Handoff authority conformance bugfix, regression-first remediation, clean-room first-run and recovery validation, independent release review, and fresh verification without publication.
- [`devswitchboard-upstream-preparation-boundary-017.json`](devswitchboard-upstream-preparation-boundary-017.json) records the single-run v0.2 upstream-preparation implementation experiment and its Revision 2 completion-routing continuation, including checkpoint reuse, explicit commit-verification policy, Codex-value evidence, independent review, historical compatibility, and fresh verification without restart, A/B run, or publication.
