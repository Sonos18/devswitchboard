...
Routing Recommendation `0.2` also requires a non-empty `surface_value`: the material result that requires the selected surface. Allowed values are:

- Chat: `intent_resolution`, `shared_context_acquisition`;
- Developer: `developer_decision`;
- Codex: `local_repository_truth`, `repository_grounding`, `implementation_execution`, `debugging`, `independent_review`, `fresh_verification`.

Values MUST match the selected surface. `surface_value` explains selection value; it does not authorize execution, replace `rationale` or the matched rule, prove developer approval, transfer phase ownership, or claim repository compatibility. Historical `0.1` records omit this v0.2-only field and remain valid.
