...
The machine verifier enforces the closed contract shape, version-specific fields, canonical excluded-capability vocabulary, explicit implementation/review strategy flags, deterministic provenance, and predecessor commit metadata. It rejects positive preparation work for recognized capability subjects explicitly named by `scope.excluded`, and rejects implementation- or review-subagent work when the corresponding approved-strategy flag is false. Negative statements that preserve an exclusion are not treated as work authorization.

Chat performs semantic acceptance for arbitrary goal contradictions, arbitrary included/excluded scope contradictions, acceptance-criteria contradictions, and meaning that is not deterministically representable. These layers supplement each other and preserve the normative rule that preparation cannot override approved goal, scope, acceptance, strategy, or source authority; deterministic checks do not claim machine-complete natural-language understanding and Chat review is not Developer technical code review.

When the policy is `true`, Chat MUST withhold an Approved Handoff for a dependent next task until it has verified the predecessor as a stable remote commit whose fresh verification corresponds to the exact committed tree. The dependent handoff then records a completed or reused `predecessor_commit_verification` gate with `predecessor_task_id`, the exact 40-character `commit_sha`, and a resolvable `evidence_source`. The `predecessor_task_id` MUST differ from the current handoff's `task_id`; a task cannot satisfy its own predecessor gate. When the policy is `false`, no mandatory predecessor commit-audit gate is created, though Chat may inspect a predecessor when a material event gives that inspection decision value. The policy never authorizes commit, push, merge, or publication.
